...
bool InteractionSystem::itemInsideDogTerritory(Dog& dog, Item& item) {
	Position i = item.getPosition();
	Size si = item.getSize();

	float rightI = i.x + si.width;
	float leftI = i.x;

	if (leftI > dog.getLimitTerritory()) {
		return true;
	}
	return false;
}
...
bool InteractionSystem::canTakeItem(Entity& player, Item& item) {
	...
	float bottomI = i.y + si.height;
	float topP = p.y;
	float topI = i.y;


	if((bottomI>=topP && topI<=bottomP) && 
		(leftI <= leftP && rightI >= leftP) || (leftI <= rightP && rightI >= rightP)) {
		return true;
	}
	return false;
}
