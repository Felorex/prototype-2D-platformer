...
bool Game::pressedF() {
	bool currentPressingF = sf::Keyboard::isKeyPressed(sf::Keyboard::F);
	if(currentPressingF && !isPressF) {
		isPressF = true;
		return true;
	}
	if(!currentPressingF) {
...
bool Game::canUsingItem() {
	if (pressedF()) {
		if (player.takenItem()) {
			item.setState(NOT_TAKEN);
			player.setTargetItem(nullptr);
			return true;
		}
		else if (interact.canTakeItem(player, item)) {
			player.setTargetItem(&item);
			item.setState(TAKING);
			return true;
		}
	}
	return false;
...
void Game::distractDog() {
	if (interact.itemInsideDogTerritory(dog, item) && item.getState() == NOT_TAKEN) {
		if (interact.canTakeItem(dog, item)) {
			dog.move(0.f);
			dog.setTargetItem(&item);
			item.setState(TAKING);
			dog.setDogState(DISTRACTED);
		}
	}
}
